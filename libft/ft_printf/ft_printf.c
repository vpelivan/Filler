...
char	*get_nonprc_str(const char *str, int j, int len, char *tmp)
{
	...
	while (str[i])
	{
		if (NONPRC1 || NONPRC2 || NONPRC3)
		{
			tmp[j] = '\0';
			(str[i] == '\n') ? tmp = ft_strcat(tmp, "\\n") : 0;
			(str[i] == '\t') ? tmp = ft_strcat(tmp, "\\t") : 0;
			(str[i] == '\r') ? tmp = ft_strcat(tmp, "\\r") : 0;
			(str[i] == '\v') ? tmp = ft_strcat(tmp, "\\v") : 0;
			(str[i] == '\a') ? tmp = ft_strcat(tmp, "\\a") : 0;
			(str[i] == '\b') ? tmp = ft_strcat(tmp, "\\b") : 0;
			(str[i] == '\f') ? tmp = ft_strcat(tmp, "\\f") : 0;
			j += 2;
		}
		tmp[j++] = str[i++];
	}
